...
class UserService:
    ...
    async def create_user(db: Client, user_data: UserCreate) -> UserResponse:
        ...
        try:
            ...
            # Truncate password to 72 UTF-8 bytes (bcrypt limit; safe for multibyte chars)
            password_bytes = user_data.password.encode('utf-8')
            if len(password_bytes) > 72:
                truncated_bytes = password_bytes[:72]
                user_data.password = truncated_bytes.decode('utf-8', errors='ignore')  # Reconstruct string
                logger.warning(f"Password truncated to 72 bytes for bcrypt compatibility: {len(password_bytes)} -> {len(truncated_bytes)} bytes")
            ...
            new_user = {
                ...
                "created_at": "now()",  # Add timestamp for consistency
                "updated_at": "now()"
            }
    ...
    async def authenticate_user(db: Client, email: str, password: str) -> UserResponse:
        ...
        try:
            ...
            # Verify password (no truncation here; hash was already truncated on create)
            ...
            logger.info(f"User authenticated successfully: {user_data['id']}")
